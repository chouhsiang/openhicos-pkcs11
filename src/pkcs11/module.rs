...
fn ensure_card(state: &mut State, slot_id: CK_SLOT_ID) -> CK_RV {
    ...
    if !slot.pcsc.is_connected() {
        ...
        if slot.pcsc.connect(&slot.reader).is_err() {
            // Reader present but no usable card (e.g. Windows Hello).
            return CKR_TOKEN_NOT_RECOGNIZED;
        }
        if p15::bind(&mut slot.pcsc, &mut slot.token).is_err() {
            slot.pcsc.disconnect();
            return CKR_TOKEN_NOT_RECOGNIZED;
        }
    ...
    for t in tmpl {
        unsafe {
            match t.type_ {
                CKA_CLASS
                    if !t.pValue.is_null()
                        && t.ulValueLen == std::mem::size_of::<CK_ULONG>() as CK_ULONG =>
                {
...
pub unsafe extern "C" fn c_get_info(p_info: *mut CK_INFO) -> CK_RV {
    with_state(|state| {
        ...
        fill_blank(&mut info.manufacturerID, "open-gpki");
        fill_blank(&mut info.libraryDescription, "open-gpki PKCS#11");
...
pub unsafe extern "C" fn c_get_token_info(
    ...
    with_state(|state| {
        ...
        info.ulMaxPinLen = if slot.token.max_pin != 0 {
            slot.token.max_pin as CK_ULONG
        } else {
            PIN_MAX
        };
        info.ulMinPinLen = if slot.token.min_pin != 0 {
            slot.token.min_pin as CK_ULONG
        } else {
            6
        };
    ...
    with_state(|state| {
        ...
        for attr in tmpl {
            let r = match attr.type_ {
                ...
                CKA_MODULUS_BITS => {
                    set_attr(attr, &(obj.modulus_bits as CK_ULONG).to_le_bytes())
                }
...
pub unsafe extern "C" fn c_find_objects_init(
    ...
    with_state(|state| {
        ...
        let objs: Vec<_> = state.slots[slot_id as usize]
            ...
            .take(MAX_OBJS)
            .map(|o| o.handle as CK_OBJECT_HANDLE)
            .collect();
        let Some(sess) = session_get_mut(state, h) else {
            return CKR_SESSION_HANDLE_INVALID;
        };
