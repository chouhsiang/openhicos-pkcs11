//! APDU layer for GPKI smart cards.
//!
//! Card-specific commands live in submodules by **card generation** (not agency):
//! - [`gen1`] — 一代卡：CLA `0x80`、保護式 VERIFY、`80 EA`/`C1` 簽章
//!   （含工商憑證與一代自然人憑證）
...
///
/// Gen1 uses the same proprietary `EA`/`C1` private op as sign (not ISO MSE/PSO),
