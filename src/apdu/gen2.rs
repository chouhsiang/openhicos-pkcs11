...
/// Static AES-wrapped master key material for 16-byte card keys.
const MASTER_ENC_BLOB_16: [u8; 16] = [
    0xee, 0xe7, 0x65, 0x09, 0x9e, 0x9b, 0x8d, 0x52, 0xa3, 0x37, 0x34, 0x76, 0x0d, 0xd0, 0x30, 0x5e,
];
const DIVERSE_KEK: [u8; 16] = [
    0x43, 0x48, 0x54, 0x20, 0x48, 0x69, 0x43, 0x4f, 0x53, 0x50, 0x4b, 0x43, 0x53, 0x23, 0x31, 0x31,
];
...
thread_local! {
    ...
    /// Cached after successful login; re-VERIFY before each sign.
...
/// Derive static SCP keys (K-ENC / K-MAC) via Diverse.
...
/// Re-open SCP and VERIFY, then SM-SELECT key DF before private ops.
...
/// User PIN VERIFY uses P2=`01` (AODF pin_ref not trusted yet).
...
mod tests {
    ...
    fn pkcs1_type2_unpad_extracts_message() {
        ...
        block[0] = 0x00;
        block[1] = 0x02;
        let msg = b"open-gpki-decrypt-test";
